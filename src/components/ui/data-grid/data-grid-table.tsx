...
const headerCellSpacingVariants = cva('', {
  variants: {
    size: {
      dense: 'h-9 px-2.5',
      /** 与 `components/ui/table` 中 TableHead 一致 */
      default: 'h-10 px-2'
    }
  },
  defaultVariants: {
...
function DataGridTableEmpty() {
  ...
  return (
    <tr>
      <td colSpan={totalColumns} className='text-muted-foreground'>
        {props.emptyMessage == null || props.emptyMessage === '' ? (
          <EmptyContent compact />
        ) : typeof props.emptyMessage === 'string' ? (
          <EmptyContent compact title={props.emptyMessage} />
        ) : (
          props.emptyMessage
        )}
      </td>
