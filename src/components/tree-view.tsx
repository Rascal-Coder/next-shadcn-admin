...
export interface TreeViewProps {
  ...
  /** 是否显示顶部搜索框，默认 true */
  showSearch?: boolean;
  /** 是否显示多选后的顶部工具栏（已选数量、批量勾选/取消），默认 true */
  showSelectionToolbar?: boolean;
  /** 是否显示行尾详情（Info 悬浮卡片及文件夹上的已选数量徽标），默认 true */
  showItemHoverCard?: boolean;
}
                    ...
                    >
                      {item.children?.map((child) => (
                        <TreeItem
                          ...
                          showItemHoverCard={showItemHoverCard}
                        />
                      ))}
                    </motion.div>
                  </CollapsibleContent>
...
export default function TreeView({
  ...
  useEffect(() => {
    if (searchQuery.trim()) {
      setExpandedIds((prev) => {
        ...
        searchExpandedIds.forEach((id) => next.add(id));
        return next;
      });
    }
  ...
  return (
    <div className='flex gap-4'>
      <div
        ref={treeRef}
        className='bg-background relative w-[600px] max-w-2xl space-y-4 rounded-xl border p-6 shadow-lg'
      >
        <AnimatePresence mode='wait'>
          {selectedIds.size > 0 && showSelectionToolbar ? (
            <motion.div
              key='selection'
        ...
        <div
          ref={dragRef}
          className={cn('bg-card relative rounded-lg select-none', className)}
